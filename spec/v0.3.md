...
AHCP is a vendor- and runtime-neutral protocol by which **agents reach humans** and **receive decisions
...
the originating (often ephemeral) agent via **push** (webhook) or **pull** (polling). AHCP is the
...
- **Agent** — a software process (any runtime) that originates AHCP messages. A "spoke."
...
The defining AHCP flow: an agent that **exits** after asking, and a **new** process that resumes when the
...
inside the `signed_context` covered by the `A2H-Signature`
...
## 11. Provenance (what AHCP reuses)
